...
public class VerleihServiceImplTest
{
	...
	@Test
	public void testVerleihstatusWirdUebernommen()
	{
		List<Medium> medien = new ArrayList<Medium>();
		medien.add(_bad);
		medien.add(_abbey);
		_verleihService.verleiheAn(_homer, medien, Datum.heute());
		assertTrue(_verleihService.istVerliehen(_bad));
		assertTrue(_verleihService.istVerliehen(_abbey));
	}

	@Test
	public void testVerlieheneMedienSindKundenZugeordnet()
	{
		List<Medium> medien = new ArrayList<Medium>();
		medien.add(_bad);
		medien.add(_abbey);
		_verleihService.verleiheAn(_homer, medien, Datum.heute());
		List<Medium> gelieheneMedien = _verleihService
				.getAusgelieheneMedienFuer(_homer);
		for (Medium medium : gelieheneMedien)
		{
